...
import (
	...
	omni "github.com/voxgig-sdk/first-news-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.FirstNewsError); is {
				out := map[string]any{
					"name":    "FirstNewsError",
...
func omniCtx(arg any, client *sdk.FirstNewsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.FirstNewsSDK); is {
			client = live

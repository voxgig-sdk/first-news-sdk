...
boru-driven command-line client **and** interactive REPL for the FirstNews
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/first-news-cli)
make build
...
./first-news-cli list new
./first-news-cli load 1 new            # {id:1} shorthand
./first-news-cli load '{id:1}' new       # explicit match map

# 5. Override the API base URL for a single call
FIRST_NEWS_BASE=https://api.example.com ./first-news-cli list new
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/first-news-cli
   ```
...
```sh
./first-news-cli load 1 new          # scalar shorthand for {id:1}
./first-news-cli load '{id:1}' new     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `first-news>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/first-news-cli`. |
...
(`./first-news-cli <expr>`) and interactively (the REPL), and expressions compose the

...
module FirstNewsOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if FirstNewsOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = FirstNewsOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| FirstNewsOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || FirstNewsOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = FirstNewsSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = FirstNewsError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { FirstNewsOmni.errify(err) }

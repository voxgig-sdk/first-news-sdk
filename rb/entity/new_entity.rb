...
class NewEntity
  ...
  def data_set(args)
    if args
      @_data = FirstNewsHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = FirstNewsHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [New, Hash] the loaded New; raises FirstNewsError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = FirstNewsHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<New>, Array] the matching New items; raises FirstNewsError on failure

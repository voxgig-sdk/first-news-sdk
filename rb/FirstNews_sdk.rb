...
require_relative 'FirstNews_types'
...
class FirstNewsSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = FirstNewsHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = FirstNewsHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, FirstNewsFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    FirstNewsUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = FirstNewsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = FirstNewsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = FirstNewsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => FirstNewsError.new(
        "#{op}_allow",
        "FirstNewsSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = FirstNewsHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = FirstNewsError.new(
        "graphql_error", "FirstNewsSDK: graphql: #{msg}")
      res["graphql"] = errors
    end

...
local FirstNewsSDK = {}
FirstNewsSDK.__index = FirstNewsSDK
...
FirstNewsSDK._make_feature = _make_feature


function FirstNewsSDK.new(options)
  local self = setmetatable({}, FirstNewsSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function FirstNewsSDK:options_map()
...
function FirstNewsSDK:get_utility()
  return Utility.copy(self._utility)
end


function FirstNewsSDK:get_root_ctx()
  return self._rootctx
end


function FirstNewsSDK:prepare(fetchargs)
...
function FirstNewsSDK:direct(fetchargs)
...
-- by passing it.
function FirstNewsSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function FirstNewsSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "FirstNewsSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function FirstNewsSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = FirstNewsSDK.new(sdkopts)
  sdk.mode = "test"

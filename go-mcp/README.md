...
MCP server exposing the FirstNews SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `first-news_list` | `entity`, optional `query` map | First page of records as JSON |
| `first-news_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `first-news_list` and
`first-news_load` tools become available in new sessions.

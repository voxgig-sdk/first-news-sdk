...
[MCP](https://modelcontextprotocol.io) server exposing the FirstNews SDK as
two agent tools — `first-news_list` and `first-news_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/first-news-mcp)
make build

# 2. Provide credentials via the environment
export FIRST_NEWS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user first-news \
  -- /absolute/path/to/first-news-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./first-news-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// first-news_list: first page of records
{ "entity": "new" }
{ "entity": "new", "query": { } }

// first-news_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/first-news-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user first-news \
     -- "$PWD"/dist/*/first-news-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `first-news_list` and `first-news_load` tools now appear
   in new sessions. Ask the agent to *"list new using first-news"*
   and it calls `first-news_list` with `{"entity":"new"}`.
...
```sh
export FIRST_NEWS_APIKEY=sk_live_xxx            # API key
export FIRST_NEWS_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `first-news_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `first-news_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FIRST_NEWS_APIKEY` | API key sent with every request. |
| `FIRST_NEWS_BASE` | Optional override of the API base URL. |
...
```sh
./first-news-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"first-news_load","arguments":{"entity":"new","query":{"id":1}}}}'
```

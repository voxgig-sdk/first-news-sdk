...
class FirstNewsTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            FirstNewsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = FirstNewsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = FirstNewsTestRunner.getenv("PROJECTENV_TEST_LIVE")
        override = FirstNewsTestRunner.getenv("PROJECTENV_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = FirstNewsTestRunner.getenv(key)
        ...
        explain = FirstNewsTestRunner.getenv("PROJECTENV_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if FirstNewsTestRunner._test_control is not None:
            return FirstNewsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                FirstNewsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            FirstNewsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = FirstNewsTestRunner.load_test_control()
...
def load_env_local():
    FirstNewsTestRunner.load_env_local()


def env_override(m):
    return FirstNewsTestRunner.env_override(m)


def entity_data(v):
    return FirstNewsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return FirstNewsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return FirstNewsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return FirstNewsTestRunner.load_test_control()


def live_client_options():
    return FirstNewsTestRunner.live_client_options()


def live_delay_ms():
    return FirstNewsTestRunner.live_delay_ms()

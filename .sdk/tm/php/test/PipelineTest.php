...
{
    private static function utility(): FirstNewsUtility
    {
        return new FirstNewsUtility();
    }
    ...
    private static function ctx(array $over = []): FirstNewsContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new FirstNewsContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new FirstNewsOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof FirstNewsError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new FirstNewsOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = FirstNewsMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new FirstNewsOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = FirstNewsMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new FirstNewsOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = FirstNewsMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FirstNewsSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = FirstNewsMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new FirstNewsResponse([]),
            'result' => new FirstNewsResult([]),
        ]);
        [, $err] = FirstNewsMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new FirstNewsSpec([]),
            'response' => null,
            'result' => new FirstNewsResult([]),
        ]);
        [, $err] = FirstNewsMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new FirstNewsSpec(['step' => 's']),
            'response' => new FirstNewsResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new FirstNewsResult([]),
        ]);
        [, $err] = FirstNewsMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new FirstNewsSpec(['step' => 's']),
            'response' => new FirstNewsResponse(self::resp(200, ['v' => 1])),
            'result' => new FirstNewsResult([]),
        ]);
        [, $err] = FirstNewsMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new FirstNewsSpec(['step' => 's']),
            'response' => new FirstNewsResponse(self::resp(200, ['v' => 2])),
            'result' => new FirstNewsResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        FirstNewsMakeResponse::call($ctx);
    ...
    {
        $preset = new FirstNewsResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new FirstNewsSpec([]),
            'response' => new FirstNewsResponse([]),
            'result' => new FirstNewsResult([]),
        ...
        [$got, $err] = FirstNewsMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new FirstNewsResult([])]);
        [, $err] = FirstNewsMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new FirstNewsSpec([]), 'result' => null]);
        [, $err] = FirstNewsMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FirstNewsSpec(['step' => 's']),
            'result' => new FirstNewsResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new FirstNewsOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FirstNewsMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FirstNewsSpec(['step' => 's']),
            'result' => new FirstNewsResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new FirstNewsOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FirstNewsMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FirstNewsResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new FirstNewsSpec([]),
            'result' => new FirstNewsResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = FirstNewsMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FirstNewsMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new FirstNewsError('boom', 'boom');
        $utility->fetcher = function (FirstNewsContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FirstNewsSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = FirstNewsMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (FirstNewsContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = FirstNewsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(FirstNewsResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (FirstNewsContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FirstNewsSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = FirstNewsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new FirstNewsResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new FirstNewsSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = FirstNewsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = FirstNewsMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = FirstNewsMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new FirstNewsResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, FirstNewsDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new FirstNewsResult(['ok' => false])]);
        $this->expectException(FirstNewsError::class);
        FirstNewsDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new FirstNewsResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', FirstNewsMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        FirstNewsMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new FirstNewsBaseFeature();
        $b = new FirstNewsBaseFeature();
        FirstNewsFeatureAdd::call($ctx, $a);
        FirstNewsFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): FirstNewsBaseFeature
    {
        $f = new FirstNewsBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        FirstNewsFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?FirstNewsSpec $spec): FirstNewsContext
    {
    ...
    private static function auth_bags(): FirstNewsSpec
    {
        return new FirstNewsSpec(['headers' => [], 'query' => []]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FirstNewsPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new FirstNewsResponse(['headers' => null]),
            'result' => new FirstNewsResult([]),
        ]);
        FirstNewsResultHeaders::call($ctx);

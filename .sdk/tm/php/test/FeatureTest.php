...
class FtCtrl extends FirstNewsControl
{
...
class FtClient
{
    ...
    private FirstNewsUtility $utility;

    public function __construct(FirstNewsUtility $utility, array $options)
    {
    ...
    public function get_utility(): FirstNewsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FirstNewsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public FirstNewsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (FirstNewsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FirstNewsConfig::make_feature($name);
    ...
    private static function buildUrl(FirstNewsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FirstNewsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FirstNewsResponse($fetched);
        }
    ...
    private function fail(FirstNewsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FirstNewsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

...
class FirstNewsRetryFeature extends FirstNewsBaseFeature
{
    ...
    public function init(FirstNewsContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (FirstNewsContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(FirstNewsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(FirstNewsContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;

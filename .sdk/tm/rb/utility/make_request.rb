...
module FirstNewsUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = FirstNewsResponse.new({})
    ...
    if fetch_err
      ...
      response = FirstNewsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FirstNewsResponse.new(fetched)
    else

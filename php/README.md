...
The PHP SDK for the FirstNews API. Provides an entity-oriented interface using PHP conventions.
...
```php
[$result, $err] = $client->New(null)->list(null, null);
...
```php
[$result, $err] = $client->New(null)->load(["id" => "example_id"], null);
if ($err) { throw new \Exception($err); }
print_r($result);
```
...
[$result, $err] = $client->FirstNews(null)->load(
    ["id" => "test01"], null
);

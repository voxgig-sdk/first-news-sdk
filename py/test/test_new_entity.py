...
class TestNewEntity:
    ...
    def test_should_stream(self):
        ...
        base = FirstNewsSDK.test(seed, None)
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set FIRST_NEWS_TEST_NEW_ENTID JSON to run live")
        client = setup["client"]
        ...
        new_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(new_ref01_data_dt0_loaded))
...
def _new_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("FIRST_NEWS_TEST_NEW_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("FIRST_NEWS_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
            },
            extra or {},
        ])
        client = FirstNewsSDK(helpers.to_map(merged_opts))

    _live = env.get("FIRST_NEWS_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("FIRST_NEWS_TEST_EXPLAIN") == "TRUE",
        "live": _live,

...
//     (\`new FirstNewsSDK(...)\` / \`FirstNewsSDK.test(...)\` become
//     \`FirstNewsSDK.test({ entity: ... })\`), transpiled to JS, and run
...
// rewrite real client construction (\`new FirstNewsSDK(...)\` and any
// \`FirstNewsSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+FirstNewsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/FirstNewsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)

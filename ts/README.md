# FirstNews TypeScript SDK

The TypeScript SDK for the FirstNews API. Provides a type-safe, entity-oriented interface with full async/await support.
...
```ts
const result = await client.New().load({ id: 'example_id' })
...
```ts
const client = new FirstNewsSDK({ apikey: '...' })
...
| `tester(testopts?, sdkopts?)` | `FirstNewsSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `FirstNewsSDK.test(testopts?, sdkopts?)` | `FirstNewsSDK` | Create a test-mode client. |
...
first-news/
├── src/
│   ├── FirstNewsSDK.ts        # Main SDK class
